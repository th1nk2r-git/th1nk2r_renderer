...
namespace {
    ...
    constexpr std::array base_color_texture_types{
        aiTextureType_BASE_COLOR,
        aiTextureType_DIFFUSE
    };
    constexpr std::array metallic_roughness_texture_types{
        aiTextureType_GLTF_METALLIC_ROUGHNESS
    };
    constexpr std::array normal_texture_types{
        aiTextureType_NORMAL_CAMERA,
        aiTextureType_NORMALS
    };
    constexpr std::array occlusion_texture_types{
        aiTextureType_AMBIENT_OCCLUSION,
        aiTextureType_LIGHTMAP
    };
    constexpr std::array emissive_texture_types{
    ...
    auto load_material_texture(
        ...
        for (const auto texture_type : texture_types) {
            if (source.GetTexture(
                    ...
                    0,
                    &texture_path
                ) == AI_SUCCESS && texture_path.length > 0) {
                found = true;
                break;
            }
        }
